...
class Resource {
  ...
  createScriptSrcTag(url) {
    // support script ext attributes
    const { crossorigin, defer } = this.config;
    let crossoriginAttr = '';
    let deferAttr = '';
    if (crossorigin) {
      crossoriginAttr = typeof crossorigin === 'string' ? ` crossorigin="${crossorigin}"` : ` crossorigin="anonymous"`;
    }
    if (defer) {
      deferAttr = ' defer';
    }
    return `<script type="text/javascript"${crossoriginAttr}${deferAttr} src="${url}"></script>`;
  }
