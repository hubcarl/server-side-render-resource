...
class Resource {
  ...
  createPreloadTag(url,tag='script') {
    if (this.config.crossorigin) {
      const crossorigin = typeof this.config.crossorigin === 'string' ? this.config.crossorigin : 'anonymous';
      return `<link rel="preload" href="${url}" crossorigin="${crossorigin}" as="${tag}">`;
    }
    return `<link rel="preload" href="${url}" as="${tag}"/>`;
  }
  ...
  inject(html, name, locals = {}, options = {}) {
    ...
    if (deps) {
      ...
      if (this.config.injectJs) {
        deps.js.forEach(url => {
          preloadJsInject.push(this.createPreloadTag(url));
      ...
      headInject = preloadJsInject.concat(headInject);
